...
let payouts = readDB("payouts", []); // décaissements admin -> Mobile Money du professionnel
...
// Configuration des comptes marchands MIMOYE pour dépôt Mobile Money manuel
// (Orange Money, MTN Money, Moov Money — en plus de Wave ci-dessus). Contrairement
// à Wave, ces opérateurs n'ont pas d'API de paiement en ligne accessible sans
// contrat entreprise spécifique : le circuit reste donc "dépôt manuel + vérification
// admin" par nature, pas une limitation temporaire. Le client dépose sur le numéro
// marchand MIMOYE (communiqué par un administrateur, jamais affiché automatiquement),
// l'admin vérifie la réception puis décaisse manuellement — après déduction de la
// commission — vers le numéro Mobile Money personnel du professionnel.
const MOBILE_MONEY_CONFIG = {
  orange_money: { label: "Orange Money", number: process.env.MM_ORANGE_NUMBER || null },
  mtn_money: { label: "MTN Money", number: process.env.MM_MTN_NUMBER || null },
  moov_money: { label: "Moov Money", number: process.env.MM_MOOV_NUMBER || null },
  wave: { label: "Wave", number: WAVE_CONFIG.paymentNumber }
};
...
const PRO_PRIVATE_FIELDS = ["tel", "whatsapp", "emailPro", "paymentInfo", "userId", "mmOperator", "mmNumber"];
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === "/api/professionals/register" && method === "POST") {
      ...
      const mmOperator = (body.mmOperator || "").trim();
      const mmNumber = (body.mmNumber || "").trim();
      if (!metier || !zone || !tel) return sendJSON(res, 400, { error: "metier, zone et tel sont requis." });
      if (mmOperator && !MOBILE_MONEY_CONFIG[mmOperator]) return sendJSON(res, 400, { error: "Opérateur Mobile Money invalide." });
      ...
      if (existing) {
        Object.assign(existing, { metier, zone, tel, whatsapp, emailPro, mmOperator, mmNumber, tarif: tarif || existing.tarif, badge: "attente" });
      } else {
        existing = { id: newId("p"), userId: user.id, name: user.name, metier, zone, tel, whatsapp, emailPro, mmOperator, mmNumber, tarif: tarif || "Non renseigné", badge: "attente", note: 0, avis: 0, init };
    ...
    // ================= PAIEMENT WAVE =================
    // Deux circuits cohabitent, sélectionnés automatiquement selon WAVE_CONFIG.liveReady :
    //
    // 1) CIRCUIT AUTOMATIQUE (WAVE_API_KEY définie) : appel réel à l'API Checkout de Wave
    //    (https://docs.wave.com/checkout). On crée une session de paiement, on renvoie au
    //    client l'URL "wave_launch_url" vers laquelle son navigateur doit être redirigé ; il
    //    paie dans l'app Wave puis Wave confirme automatiquement via webhook signé
    //    (POST /api/webhooks/wave, événement "checkout.session.completed") — voir
    //    finalizeWavePayment ci-dessous, partagée avec la confirmation admin manuelle.
    //
    // 2) CIRCUIT MANUEL DE SECOURS (pas de clé API) : WAVE_PAYMENT_NUMBER est un vrai numéro
    //    marchand Wave Business. Le client envoie réellement l'argent via l'app Wave sur ce
    //    numéro, indique la référence de sa transaction, et un administrateur MIMOYE confirme
    //    après vérification manuelle dans son application Wave. Ce n'est pas une simulation :
    //    c'est un vrai paiement, avec un contrôle humain au lieu d'un webhook automatique.
    ...
    if (payInitMatch && method === "POST") {
      ...
      if (WAVE_CONFIG.liveReady) {
        ...
        try {
          ...
          reqObj.payment = { status: "PENDING_CONFIRMATION", mode: "wave_api", montant, reference: session.id, waveLaunchUrl: session.wave_launch_url, initiatedAt: Date.now() };
    ...
    // Utilisée à la fois par le webhook Wave (confirmation automatique) et par
    // /api/admin/requests/:id/confirm-payment (confirmation manuelle) pour garantir
    // que le calcul de commission et le crédit du portefeuille pro suivent toujours
    // exactement la même logique, quel que soit le circuit de paiement emprunté.
    function finalizeWavePayment(reqObj, confirmedByUserId) {
      const pro = professionals.find(p => p.id === reqObj.professionalId);
      const montant = reqObj.payment.montant;
      const moyenLabel = reqObj.payment.operatorLabel || "Wave";
      ...
      if (pro) {
        ...
        notify(pro.userId, "payment", `Paiement ${moyenLabel} confirmé pour une prestation : ${montantPro} FCFA seront crédités à votre portefeuille (commission MIMOYE : ${commissionAmount} FCFA).`);
      }
      notify(reqObj.clientId, "payment", `Votre paiement ${moyenLabel} de ${montant} FCFA a été confirmé.`);
      log("payment_confirmed", confirmedByUserId, { requestId: reqObj.id, montant, commissionAmount, montantPro, mode: reqObj.payment.mode });
      saveAll();
    }
    ...
    if (payConfirmClientMatch && method === "POST") {
      ...
      notify(null, "payment", `Un client déclare avoir payé ${reqObj.payment.montant} FCFA via ${moyenLabel} pour la demande ${reqObj.id} — référence indiquée : ${reqObj.payment.clientReference || "non fournie"}. Vérification requise.`);
      log("payment_declared", user.id, { requestId: reqObj.id, reference: reqObj.payment.clientReference, mode: reqObj.payment.mode });
      saveAll();
      return sendJSON(res, 200, { ok: true, request: reqObj });
    }
    ...
    if (payConfirmAdminMatch && method === "POST") {
      ...
      finalizeWavePayment(reqObj, user.id);
      return sendJSON(res, 200, { ok: true, request: reqObj });
    }
    ...
    if (payRejectAdminMatch && method === "POST") {
      ...
      log("payment_rejected", user.id, { requestId: reqObj.id, reason: body.reason || "" });
      saveAll();
      return sendJSON(res, 200, { ok: true, request: reqObj });
    }

    // ================= PAIEMENT MOBILE MONEY (Orange/MTN/Moov) — dépôt manuel sur compte marchand,
    // numéro communiqué UNIQUEMENT par un administrateur (jamais affiché automatiquement au client),
    // puis décaissement admin vers le Mobile Money personnel du professionnel après déduction de
    // la commission. Contrairement à Wave, ces opérateurs n'ont pas d'API de paiement disponible
    // sans contrat entreprise dédié : ce circuit manuel est la solution normale, pas provisoire.
    const mmRequestMatch = pathname.match(/^\/api\/requests\/([^\/]+)\/pay-request-mobile-money$/);
    if (mmRequestMatch && method === "POST") {
      const user = requireAuth(); if (!user) return;
      const reqObj = requests.find(r => r.id === mmRequestMatch[1]);
      if (!reqObj) return sendJSON(res, 404, { error: "Demande introuvable." });
      if (reqObj.clientId !== user.id) return sendJSON(res, 403, { error: "Accès refusé." });
      if (reqObj.status !== "QUOTE_ACCEPTED") return sendJSON(res, 400, { error: "Aucun devis accepté à payer pour cette demande." });
      const body = await readBody(req);
      const operator = body.operator;
      if (!MOBILE_MONEY_CONFIG[operator]) return sendJSON(res, 400, { error: "Opérateur invalide. Choisissez Orange Money, MTN Money ou Moov Money." });
      const montant = Number(reqObj.quote && reqObj.quote.amount) || 0;
      reqObj.payment = { status: "PENDING_INSTRUCTIONS", mode: operator, operatorLabel: MOBILE_MONEY_CONFIG[operator].label, montant, reference: newId("txn"), initiatedAt: Date.now() };
      notify(null, "payment", `Un client souhaite payer ${montant} FCFA via ${MOBILE_MONEY_CONFIG[operator].label} pour la demande ${reqObj.id}. Merci de lui communiquer le numéro marchand MIMOYE correspondant.`);
      log("payment_mm_requested", user.id, { requestId: reqObj.id, operator, montant });
      saveAll();
      return sendJSON(res, 200, { ok: true, request: reqObj, note: "Un administrateur MIMOYE va vous communiquer sous peu le numéro marchand sur lequel effectuer votre dépôt." });
    }
    if (pathname === "/api/admin/payments/awaiting-instructions" && method === "GET") {
      const user = requireAuth(); if (!user) return;
      if (!requireRole(user, ["admin"])) return;
      return sendJSON(res, 200, requests.filter(r => r.payment && r.payment.status === "PENDING_INSTRUCTIONS"));
    }
    const mmSendInstrMatch = pathname.match(/^\/api\/admin\/requests\/([^\/]+)\/send-payment-instructions$/);
    if (mmSendInstrMatch && method === "POST") {
      const user = requireAuth(); if (!user) return;
      if (!requireRole(user, ["admin"])) return;
      const reqObj = requests.find(r => r.id === mmSendInstrMatch[1]);
      if (!reqObj || !reqObj.payment || reqObj.payment.status !== "PENDING_INSTRUCTIONS") return sendJSON(res, 400, { error: "Aucune demande d'instructions en attente pour cette demande." });
      const operatorConf = MOBILE_MONEY_CONFIG[reqObj.payment.mode];
      const merchantNumber = (operatorConf && operatorConf.number) || "";
      if (!merchantNumber) return sendJSON(res, 400, { error: `Aucun numéro marchand ${operatorConf ? operatorConf.label : ""} configuré (variable d'environnement manquante).` });
      reqObj.payment.merchantNumber = merchantNumber;
      reqObj.payment.status = "PENDING_CONFIRMATION";
      reqObj.payment.instructionsSentAt = Date.now();
      notify(reqObj.clientId, "payment", `MIMOYE vous communique le numéro marchand ${reqObj.payment.operatorLabel} pour votre paiement de ${reqObj.payment.montant} FCFA : ${merchantNumber}. Une fois le dépôt effectué, indiquez la référence de votre transaction.`);
      log("payment_instructions_sent", user.id, { requestId: reqObj.id, operator: reqObj.payment.mode, merchantNumber });
      saveAll();
      return sendJSON(res, 200, { ok: true, request: reqObj });
    }

    // ================= DÉCAISSEMENT ADMIN → MOBILE MONEY DU PROFESSIONNEL =================
    // L'admin choisit de décaisser (avant ou après service fait) après avoir lui-même
    // effectué le dépôt réel dans l'app Mobile Money vers le numéro personnel du
    // professionnel — l'enregistrement ici trace cette action, déduit la commission
    // du montant du portefeuille, et prévient le professionnel.
    if (pathname === "/api/admin/payouts/candidates" && method === "GET") {
      const user = requireAuth(); if (!user) return;
      if (!requireRole(user, ["admin"])) return;
      const list = professionals.filter(p => p.userId).map(p => ({
        professionalId: p.id, name: p.name, mmOperator: p.mmOperator || null, mmNumber: p.mmNumber || null,
        wallet: getWallet(p.id)
      })).filter(x => x.wallet.available > 0 || x.wallet.pending > 0);
      return sendJSON(res, 200, list);
    }
    if (pathname === "/api/admin/payouts" && method === "GET") {
      const user = requireAuth(); if (!user) return;
      if (!requireRole(user, ["admin"])) return;
      return sendJSON(res, 200, payouts.slice().reverse());
    }
    if (pathname === "/api/admin/payouts" && method === "POST") {
      const user = requireAuth(); if (!user) return;
      if (!requireRole(user, ["admin"])) return;
      const body = await readBody(req);
      const pro = professionals.find(p => p.id === body.professionalId);
      ...
      const timing = body.timing === "avant_service" ? "avant_service" : "apres_service";
      if (!montant || montant <= 0) return sendJSON(res, 400, { error: "Montant invalide." });
      if (!pro.mmNumber) return sendJSON(res, 400, { error: "Ce professionnel n'a pas encore renseigné de numéro Mobile Money pour être payé." });
      const w = getWallet(pro.id);
      // "Avant service" décaisse depuis les fonds encore en attente de déblocage (avance
      // exceptionnelle) ; "après service" depuis le solde normalement disponible.
      const source = timing === "avant_service" ? "pending" : "available";
      if (w[source] < montant) return sendJSON(res, 400, { error: `Solde ${source === "pending" ? "en attente" : "disponible"} insuffisant (${w[source]} FCFA).` });
      w[source] -= montant;
      const payout = { id: newId("payout"), professionalId: pro.id, montant, timing, mmOperator: pro.mmOperator || null, mmNumber: pro.mmNumber, adminId: user.id, reference: body.reference || "", at: Date.now() };
      payouts.push(payout);
      notify(pro.userId, "payment", `MIMOYE a envoyé ${montant} FCFA sur votre compte Mobile Money (${pro.mmOperator ? MOBILE_MONEY_CONFIG[pro.mmOperator].label : "—"}) — décaissement ${timing === "avant_service" ? "avant intervention" : "après intervention"}.`);
      log("payout_sent", user.id, { professionalId: pro.id, montant, timing, reference: body.reference || "" });
      saveAll();
      return sendJSON(res, 200, { ok: true, payout, wallet: w });
    }
    ...
    if (pathname === "/api/admin/payment-config" && method === "GET") {
      ...
      return sendJSON(res, 200, {
        ...
        mobileMoney: Object.keys(MOBILE_MONEY_CONFIG).reduce((acc, k) => {
          acc[k] = { label: MOBILE_MONEY_CONFIG[k].label, configured: !!MOBILE_MONEY_CONFIG[k].number };
          return acc;
        }, {})
      });
    }
    ...
    if (pathname === "/api/webhooks/wave" && method === "POST") {
      ...
      if (event && event.type === "checkout.session.completed" && data) {
        ...
        if (reqObj && reqObj.payment && reqObj.payment.status === "PENDING_CONFIRMATION") {
          finalizeWavePayment(reqObj, null);
        }
